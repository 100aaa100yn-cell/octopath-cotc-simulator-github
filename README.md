...
Game8の個別キャラクターページを2026年7月22日に確認し、次の剣士5人について各10件、合計50件のバトルアビリティを標準データへ登録しました。
...
技変化、BP5消費、カウントダウン、追加発動など、現在の戦闘エンジンで完全再現できない条件は説明欄へ保持しています。通常時のSP・威力・Hit数・攻撃属性はシミュレーター計算へ反映されます。
...
ターン戦闘の結果から、撃破可否、低火力ターン、SP不足、ブレイクと最大火力のずれ、余剰BP、盾削り速度、ダメージ偏重を分析し、優先度付きの改善案を表示します。提案はJSONとして出力できます。


### v2.8 複数プラン比較

戦闘結果を保存し、最大4件の撃破ターン、ダメージ、SP効率、ブレイク回数、改善スコアを横並びで比較できます。


## v2.9 勝ち筋探索

保存中の攻略プランから複数の改善候補を生成し、Boost配分、ブレイク速度、SP効率、交代周期を簡易評価します。候補は新しい攻略プランとして適用でき、元プランは保持されます。候補適用後はターン戦闘を実行し、複数プラン比較で実測結果を確認してください。
...
## v3.2 Game8 バトルアビリティインポーター

Game8のキャラクター個別ページ全文を貼り付け、バトルアビリティ名、SP、威力、Hit数、Boost値、説明文、出典URLを解析します。解析プレビュー後に追加・更新または既存バトル技の置換を選べます。ブラウザのCORS制限とサイト負荷を避けるため、URLへの自動巡回ではなくページ本文貼付方式です。

1. Game8で対象キャラクターの個別ページを開く
2. Ctrl+A、Ctrl+Cでページ全文をコピー
3. 本ツールの「Game8 バトルアビリティ取込」へ貼り付ける
4. 「解析して差分確認」後、「解析結果を登録」

取り込んだデータにはGame8 URL、確認日、元説明文を保持します。特殊効果は元説明文を優先し、シミュレーターが理解できる基本項目のみ自動構造化します。
